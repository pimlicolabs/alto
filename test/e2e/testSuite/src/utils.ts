...
import { PublicClient, TestClient, createWalletClient, getContract, http, parseEther, parseGwei } from "viem"
...
export const compressAndSendOp = async (userOperation: UserOperation, publicClient: PublicClient) => {
...
export const setupSimpleSmartAccountClient = async (bundlerClient: PimlicoBundlerClient, publicClient: PublicClient) => {
    ...
    await fundAccount(simpleAccount.address, parseEther("100"))
    ...
    return createSmartAccountClient({
        ...
        sponsorUserOperation: async (args: {
            ...
            const gasInfo = await bundlerClient.estimateUserOperationGas({
                userOperation: {...args.userOperation, nonce: await simpleAccount.getNonce()},
