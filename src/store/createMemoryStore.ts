...
const addSubmitted = ({
    ...
    logger.debug(
        {
            userOpHash: op.userOperation.hash,
            store: "submitted"
        },
...
const removeOutstanding = ({
    ...
    const index = outstanding.findIndex((op) => op.hash === userOpHash)
    if (index === -1) {
        logger.warn(
...
const removeProcessing = ({
    ...
    const index = processing.findIndex((op) => op.hash === userOpHash)
    if (index === -1) {
        logger.warn(
    ...
    return {
        ...
        process({ maxTime, maxGasLimit, immediate }, callback) {
            const getOpsToBundle = () => {
                let gasUsed = 0n
                const filteredOps = this.outstanding.filter((op) => {
                    const opGasLimit =
                        op.callGasLimit +
            ...
            const processOps = () => {
                ...
                if (filteredOps.length > 0) {
                    const removeHashes = new Set(
                        filteredOps.map((op) => op.hash)
                    )
                    this.outstanding = this.outstanding.filter(
                        (opInfo) => !removeHashes.has(opInfo.hash)
                    )
